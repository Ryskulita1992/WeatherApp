    ...
    var body: some View {
        VStack(spacing: 10){
            Text ("Today")
                ...
                .bold()
            HStack(spacing: 20){
                LottieView(name: cityVM.getLottieAnimFor(icon: cityVM.weatherIcon))
            ...
            HStack{
                Spacer()
                widgetView(image: "wind", color: .gray
                           , title: "\(cityVM.windSpeed)mi/hr")
                Spacer()
                widgetView(image: "drop.fill", color: .blue, title:  "\(cityVM.humiduty)")
                Spacer()
